...
#define TEST_TARGET(rng_type, rng_size)                       pcg_       ## rng_type ## _ ## rng_size ## _srandom_r
...
#define TEST_CODE_CORE(rng_type, rng_size) \
    do { \
        TEST_TARGET(rng_type, rng_size)(&rng, state); \
...
#define TEST_CODE_CORE_SETSEQ(rng_type, rng_size) \
    do { \
        TEST_TARGET(setseq, rng_size)(&rng, state, TARGET_PCG_STATE_INITIALIZER(setseq, rng_size).inc); \
    ...
    do { \
        ...
        { \
            ...
            if (rng_size < 32) { \
                for (TARGET_UNSIGNED_INT(rng_size) state = 0;;) \
                { \
                    TEST_CODE_CORE_SETSEQ(rng_type, rng_size) \
                    if (state == TARGET_UNSIGNED_INT_MAX(rng_size)) {break;} \
                    state++; \
                } \
            } \
